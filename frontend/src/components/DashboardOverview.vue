...
const emit = defineEmits<{ navigate: [page: 'bots' | 'brain' | 'chat' | 'rules' | 'settings']; toggle: []; refresh: [] }>()
...
const status = computed(() => dashboardStatus(props))
function act() {
  const action = status.value.action
  if (action === 'toggle') emit('toggle')
...
<template>
  <div class="dashboard-home">
    ...
    <section class="stream-hero" :class="{ 'stream-hero-warning': status.warning }">
      <div class="hero-copy"><span class="hero-kicker"><i :class="{ live }"></i>{{ status.label }}</span><h2>{{ status.title }}</h2><p>{{ status.text }}</p><button class="hero-action" :disabled="busy" @click="act">{{ status.actionLabel }} <span aria-hidden="true">↗</span></button></div>
    ...
    <div class="home-columns">
      ...
      <aside class="home-right">
        ...
        <section class="feedback-callout"><span class="feedback-icon" aria-hidden="true">✦</span><h2>Ваши оценки помогают</h2><p>Оценивайте сообщения ботов — по вашим замечаниям ИИ предложит правила. Их можно проверить и отключить.</p><button @click="emit('navigate', 'rules')">{{ pendingFeedback ? `${pendingFeedback} оценок ждут разбора` : 'Как работает обучение' }} <span aria-hidden="true">→</span></button></section>
