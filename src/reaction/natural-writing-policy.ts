...
export const REACTION_NATURALNESS_INSTRUCTIONS = [
  ...
  'Speech lists — favourite forms, openings, endings, fillers, laughs — are tendencies, not slots. A message containing none of them is the common case, and a message assembled out of several of them is never right. A laughStyle in particular is how this person laughs when something is genuinely funny, not a prefix.',
  'Name things the way the stream just named them. Preserve clearly heard wording from recentSpeech instead of inventing clipped forms the reader has to decode. Never invent slang for something that was said plainly.',
  'Check a draft against that account’s own recentMessages before emitting it. If it repeats an opening, an address, a catchphrase, a sentence frame, a joke or a close paraphrase they have already used, write it differently.',
  'Never carry a transcript label into a message. "S:" and "O:" mark tentative speaker roles; "U:" means unknown; nobody typing in chat writes those.',
] as const;
